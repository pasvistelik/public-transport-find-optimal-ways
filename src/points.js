        ...
        for (var selectedPoint = this.finalPoint.previousPoint, selectedPointStation, nextPoint, nextStation; ; selectedPoint = nextPoint){
            ...
            if (table.type === TableType.table) // Если это точное расписание, то:
            {
                ...
                if (newDistance < minimalDistance){
                    ...
                    console.log("["+selectedRoute.type+" "+selectedRoute.number+"][+"+appendedTimeSeconds+" seconds, -"+appendedGoingTime+" going seconds, k = "+(appendedGoingTime/appendedTimeSeconds).toFixed(2)+"]: Can change "+oldStation.name+"("+oldDistance+") to "+nextStation.name+"("+minimalDistance+")");
                }
            }
            else if (table.type === TableType.periodic) {
                throw new Error();
