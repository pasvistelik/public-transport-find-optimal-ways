    ...
    optimizeFindedWay(day, time, reservedTime, speed) {
        ...
        for (var selectedPoint = this.finalPoint.previousPoint, selectedPointStation, nextPoint, nextStation; ; selectedPoint = nextPoint){
            ...
            if (table.type === TableType.table) // Если это точное расписание, то:
            {
                ...
                if (oldTotalTimeSeconds < arrivalTime + newGoingTimeFromNewToFinal + 600) break;
        ...
        if (minimalDistance < oldDistance) console.log("["+selectedRoute.type+" "+selectedRoute.number+"][+"+appendedTimeSeconds+" seconds]: Can change "+oldStation.name+"("+oldDistance+") to "+NewStation.name+"("+minimalDistance+")");
        //return;
