...
var Points = function () {
    ...
    (0, _createClass3.default)(Points, [{
        ...
        value: function countShortWay(ignoringRoutes, myIgnoringFragments, time, types, speed, reservedTime) {
            ...
            for (var currentPoint = this.finalPoint.previousPoint, _selectedRoute = currentPoint.fromWhichRoute, previousPoint = currentPoint.previousPoint; currentPoint !== this.startPoint; currentPoint = currentPoint.previousPoint, _selectedRoute = currentPoint.fromWhichRoute, previousPoint = currentPoint.previousPoint) {

                // Рассматриваем вершину только если использованные маршруты не совпадают. Пути "пешком" не рассматриваем.
                if (_selectedRoute == null || previousPoint === this.startPoint || previousPoint.fromWhichRoute === _selectedRoute) continue;

                var previousStationOfSelectedRoute = _selectedRoute.getPreviousStation(previousPoint.station);
                ...
                // Загружаем расписание:
                var table = _selectedRoute.getTimetable(point.station);
                if (table == null) continue;
                ...
                if (table.type === TableType.table) // Если это точное расписание, то:
                    {
                        ...
                        if (momentOfDispatchFromPoint >= point.totalTimeSeconds + reservedTime && point.getTotalGoingTime() <= previousPoint.getTotalGoingTime()) {
                            ...
                            previousPoint.fromWhichRoute = _selectedRoute;
