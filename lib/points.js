...
var Points = function () {
    ...
    _createClass(Points, [{
        ...
        value: function optimizeFindedWay(day, time, reservedTime) {
            var tmp = [];
            //console.log("\n\n\nTry optimize...");
            ...
            for (var nextPoint = this.finalPoint.previousPoint, selectedRoute = nextPoint.fromWhichRoute, _currentPoint = nextPoint.previousPoint; nextPoint !== this.startPoint; nextPoint = nextPoint.previousPoint, selectedRoute = nextPoint.fromWhichRoute, _currentPoint = nextPoint.previousPoint) {

                // Рассматриваем вершину только если использованные маршруты не совпадают. Пути "пешком" не рассматриваем.
                if (selectedRoute == null || _currentPoint === this.startPoint || _currentPoint.fromWhichRoute === selectedRoute) continue;

                var previousStationOfSelectedRoute = selectedRoute.getPreviousStation(_currentPoint.station); //!!! currentPoint
                ...
                if (table.type === TableType.table) // Если это точное расписание, то:
                    {
                        ...
                        var momentOfDispatchFromPoint = _currentPoint.dispatchTime + table.findTimeBefore(time + _currentPoint.dispatchTime, day);
                        ...
                        if (momentOfDispatchFromPoint >= point.arrivalTime + reservedTime) {
                            ...
                            var p2 = _currentPoint.getTotalGoingTime(); //!!! currentPoint
                            tmp.push({
                                /*point: currentPoint,
                            ...
                            if (p1 <= p2) {
                                ...
                                _currentPoint.totalTimeSeconds = _currentPoint.dispatchTime;

                                //console.log("\n"+currentPoint.arrivalTime+" to "+currentPoint.totalTimeSeconds);
                                _currentPoint.arrivalTime = _currentPoint.dispatchTime; //currentPoint.totalTimeSeconds;//!!!!!!!!!!!!!
                                ...
                                _currentPoint.previousPoint = point;
                                _currentPoint.fromWhichRoute = selectedRoute;
                                _currentPoint.fromWhichStation = previousStationOfSelectedRoute; //point.station;
                            }
                        }
        ...
        value: function fixTimeAttributes(day, time, reservedTime) {
            ...
            if (this.finalPoint.previousPoint == null) return;
            // Сокращаем время ходьбы пешком до минимума и избавляемся от "бессмысленных" пересадок, сохраняя общее время неизменным:
            for (var nextPoint = this.finalPoint.previousPoint, selectedRoute = nextPoint.fromWhichRoute, _currentPoint2 = nextPoint.previousPoint; nextPoint !== this.startPoint; nextPoint = nextPoint.previousPoint, selectedRoute = nextPoint.fromWhichRoute, _currentPoint2 = nextPoint.previousPoint) {

                // Рассматриваем вершину только если использованные маршруты не совпадают. Пути "пешком" не рассматриваем.
                if (selectedRoute == null || _currentPoint2 === this.startPoint /*|| currentPoint.fromWhichRoute === selectedRoute*/) continue;

                // Загружаем расписание:
                var table = selectedRoute.getTimetable(_currentPoint2.station);
                if (table == null) continue;
                ...
                if (table.type === TableType.table) // Если это точное расписание, то:
                    {
                        ...
                        _currentPoint2.dispatchTime = momentOfDispatchFromPoint;
                        if (_currentPoint2.fromWhichRoute != selectedRoute) {
                            _currentPoint2.totalTimeSeconds = _currentPoint2.arrivalTime + reservedTime;
                        } else {
                            _currentPoint2.arrivalTime = momentOfDispatchFromPoint;
                            _currentPoint2.totalTimeSeconds = momentOfDispatchFromPoint;
                        }
                    } else {
                    //if (table.type === TableType.periodic)
                    throw new Error();
