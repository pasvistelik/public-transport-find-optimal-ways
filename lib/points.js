...
var Points = function () {
    ...
    _createClass(Points, [{
        ...
        value: function optimizeFindedWay(day, time, reservedTime, goingSpeed) {
            ...
            var minimalDistance = (0, _geoCoordsDistance2.default)(selectedPoint.coords, this.finalPoint.coords);
            var oldDistance = minimalDistance;
            for (var _selectedPoint = this.finalPoint.previousPoint;;) {
                var _selectedPointStation = _selectedPoint.station;
                var nextStation = selectedRoute.getNextStation(_selectedPointStation);
                if (nextStation == null) break;
                var _nextPoint2 = this.findElement(nextStation);
                var dist = (0, _geoCoordsDistance2.default)(_nextPoint2.coords, this.finalPoint.coords);
                console.log("Check to change " + oldDistance + " to " + minimalDistance);
                if (dist < minimalDistance) {
                    minimalDistance = dist;
                }
                _selectedPoint = _nextPoint2;
            }
            console.log("!!!!!!!!!!! Can change " + oldDistance + " to " + minimalDistance);
            return; //11111111111
            if (oldDistance > minimalDistance) {
                for (var _selectedPoint2 = this.finalPoint.previousPoint;;) {

                    if ((0, _geoCoordsDistance2.default)(_selectedPoint2.coords, this.finalPoint.coords) == minimalDistance) break;

                    var selectedPointStation = _selectedPoint2.station;
                    // Следующая остановка у данного транспорта:
                    ...
                    if (_nextStation != null) // Если остановка не является конечной, то:
                        {
                            ...
                            if (table.type === TableType.table) // Если это точное расписание, то:
                                {
                                    // Момент, когда мы сядем в транспорт:
                                    var momentWhenSitInTransport = time + _selectedPoint2.totalTimeSeconds;
                                    ...
                                    var arrivalTime = _selectedPoint2.totalTimeSeconds + goingOnTransportTime;
                                    var onNextPointTotalTimeSeconds = arrivalTime;

                                    var nextPoint = this.findElement(_nextStation);

                                    nextPoint.fromWhichRoute = selectedRoute;
                                    nextPoint.previousPoint = _selectedPoint2;
                                    ...
                                    nextPoint.totalGoingTimeSeconds = _selectedPoint2.totalGoingTimeSeconds;
